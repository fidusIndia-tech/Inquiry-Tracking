            ...
            {activeMenu === "dashboard" && (
              <div className="flex flex-col gap-4 pt-4 lg:pt-5">
                ...
                <InquiryTable
                  ...
                  onAddInquiry={() => handleOpenAddModal(null)}
                />
              </div>
            )}
...
function InquiryTable({
  ...
  totalCount,
  now,
  onDeleteRequest, onEditRequest, onSubjectOpen, onDetailOpen, onAssignToMeRequest, onAutoAssign, onAddInquiry,
}) {
      ...
      <div className="flex flex-col gap-3 border-b border-[#D8E3F8] px-5 py-4 lg:flex-row lg:items-center lg:justify-between rounded-t-2xl overflow-hidden" style={{ background: "linear-gradient(90deg,#F5F8FF 0%,#F0F6FF 100%)" }}>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 h-7 rounded-lg px-3 text-[11px] font-semibold text-white transition hover:opacity-90"
            style={{ background: "linear-gradient(135deg,#5BA7FF,#6D7CFF)", boxShadow: "0 1px 6px rgba(91,167,255,0.32)" }}
            title="Manually create a new inquiry"
          >
            <Plus size={12} /> Add Inquiry
          </button>
          <button
...
function AddInquiryModal({ reminder, employees = [], onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-950/30 backdrop-blur-sm" onClick={onClose} />
      <div className="relative z-10 w-full max-w-3xl bg-white rounded-2xl card-shadow-lg animate-modal overflow-hidden max-h-[92vh] flex flex-col">

        {/* Header */}
        <div className="flex items-start justify-between border-b border-[#EEF2F6] px-5 py-4" style={{ background: "linear-gradient(90deg,#F5F8FF,#F0F6FF)" }}>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Admin · Manual Entry</p>
            ...
            <div className="space-y-1.5">
              {items.map((item, i) => (
                <div key={i} className="grid grid-cols-[1.2fr_1.2fr_1.8fr_60px_70px_28px] gap-1.5 items-center">
                  ...
                  <button onClick={() => removeItem(i)} disabled={items.length === 1} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-300 transition hover:bg-[#FFF1F2] hover:text-rose-500 disabled:opacity-30 disabled:pointer-events-none">
                    <Trash2 size={12} />
                  </button>
                </div>
              ))}
        ...
        <div className="flex items-center gap-2 border-t border-[#EEF2F6] px-5 py-4">
          ...
          <button
            ...
            style={{ background: "linear-gradient(135deg,#5BA7FF 0%,#6D7CFF 100%)", boxShadow: "0 2px 8px rgba(91,167,255,0.28)" }}
          >
            {submitting ? <><RefreshCw size={12} className="animate-spin" />Creating…</> : <><Plus size={12} />Create Inquiry</>}
          </button>
        </div>
      </div>
    </div>
